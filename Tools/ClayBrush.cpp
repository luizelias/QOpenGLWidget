...
void ClayBrush::press(const QPoint & mouse)
{
    Brush::press(mouse);
}

void ClayBrush::drag(const QPoint & mouse)
{
    ...
    for(QPoint & p: _pixels)
    {
        ...
        // set falloff
        GPU::getInstance()->picking(_mesh, radius);
        ...
        // sculpt
        Shader * shader = ShaderManager::getInstance()->getShader(Shader::kClayBrush);

        Brush::squareAlpha->bind();
...
ToolType ClayBrush::getType()
{
    return ToolType::kClayTool;
}
